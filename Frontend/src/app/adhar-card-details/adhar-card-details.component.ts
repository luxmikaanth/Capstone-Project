...
import { AdharModel } from './adhar-card-details.model';

@Component({
  selector: 'app-adhar-card-details',
  templateUrl: './adhar-card-details.component.html',
  styleUrls: ['./adhar-card-details.component.css']
})
export class AdharCardDetailsComponent implements OnInit {
  ...
  deleteadhar(row: any){
    this.api.deleteAdhar(row.adharCardId)
    .subscribe(res=>{
      alert("AdharDetail Deleted")
      this.getAllAdhar();
    })
